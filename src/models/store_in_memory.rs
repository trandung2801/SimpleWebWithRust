...
impl StoreMethods for InMemoryStore {
        ...
        -> Result<User, Error>
    {
        ...
        match self.users.write().await.get_mut(&user_update.id.clone().unwrap()) {
            Some(u) => {
                *u = user_update.clone();
                Ok(user_update)
            },
    ...
    async fn get_company_by_email(&self, company_email: String)
        -> Result<Company, Error>
    {
        let vec_company =  self.companies
            .read()
            .await
            .iter()
            .filter_map(|(_k, v)| {
    ...
    async fn update_company(&self, company: Company)
        -> Result<Company, Error>
    {
        match self.companies.write().await.get_mut(&company.id.clone().unwrap()) {
            Some(value) => {
                *value = company.clone();
                Ok(company)
            },
    ...
    async fn get_list_resume_by_user_id(&self, limit: Option<i32>, offset: i32, user_id: UserId) -> Result<Vec<Resume>, Error> {
          Ok(self.resumes
            .read()
            .await
            .iter()
    ...
    async fn update_resume(&self, resume: Resume) -> Result<Resume, Error> {
        match self.resumes.write().await.get_mut(&resume.id.clone().unwrap()) {
            Some(value) => {
                *value = resume.clone();
                Ok(resume)
            },
