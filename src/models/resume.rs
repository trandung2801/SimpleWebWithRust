...
pub trait ResumeActions {
    async fn create(store: &Arc<dyn StoreMethods>, new_resume: NewResume)
                    -> Result<Resume, Error>;
    async fn get_by_user_id(store: &Arc<dyn StoreMethods>, user_id: UserId)
                            -> Result<Resume, Error>;
    async fn get_by_id(store: &Arc<dyn StoreMethods>, resume_id: ResumeId)
                       -> Result<Resume, Error>;
    async fn list_by_user_id(store: &Arc<dyn StoreMethods>, limit: Option<i32>, offset: i32, user_id: UserId)
                  -> Result<Vec<Resume>, Error>;
    async fn list_by_job_id(store: &Arc<dyn StoreMethods>, limit: Option<i32>, offset: i32, job_id: JobId)
                             -> Result<Vec<Resume>, Error>;
    async fn update(store: &Arc<dyn StoreMethods>, resume: Resume)
                    -> Result<Resume, Error>;
    async fn delete(store: &Arc<dyn StoreMethods>, resume_id: ResumeId)
                    -> Result<bool, Error>;
}
